...
function AddressCard({item,setShowForm,setCurrentEditedId,setFormData,handleDeleteAddress,setAddressSelected}) {
  return (
    <Card className="relative p-2 shadow-md border rounded-2xl" onClick={()=>setAddressSelected(item)}>
              ...
              <CardContent className="p-2 space-y-1 text-sm overflow-hidden">
                <Label className='line-clamp-2'><span>Address:</span> <span className="text-muted-foreground">{item?.address}</span></Label>
